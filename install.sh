...
实例参数:
  ...
  --registry <addr>                 默认: sealos.hub:5000
...
prepare_runtime_auth_secret() {
  ...
  case "${ACTION}" in
    backup|restore|verify-backup-restore)
      die "动作 ${ACTION} 的认证信息无效：请提供 --mysql-password，或确保 --mysql-auth-secret/--mysql-password-key 指向已存在的 Secret 键。"
      ;;
  esac
...
validate_inputs() {
  ...
  case "${ACTION}" in
    backup|restore|verify-backup-restore)
      if [[ "${MYSQL_PASSWORD_EXPLICIT}" != "true" && "${MYSQL_AUTH_SECRET_EXPLICIT}" != "true" ]]; then
        die "动作 ${ACTION} 必须显式提供 --mysql-password 或 --mysql-auth-secret（可配合 --mysql-password-key）"
      fi
      if [[ "${MYSQL_HOST_EXPLICIT}" != "true" ]] && ! resource_exists statefulset "${STS_NAME}"; then
        die "动作 ${ACTION} 未检测到 StatefulSet/${STS_NAME}，请显式提供 --mysql-host 指向外部 MySQL"
      fi
      ;;
  esac
...
follow_benchmark_job() {
  local job_name="$1"
  local pod_name="" last_summary="" last_log=""
  ...
  while true; do
    ...
    if [[ -n "${pod_name}" ]]; then
      ...
      local current_log
      current_log="$(kubectl logs -n "${NAMESPACE}" "pod/${pod_name}" -c mysql-benchmark --tail=10 2>/dev/null || true)"
...
precheck_mysql_connectivity() {
  section "预检 MySQL 连通性"
  mysql_exec "SELECT 1;" >/dev/null
  success "MySQL 连通性预检通过: ${MYSQL_HOST}:${MYSQL_PORT}"
}
